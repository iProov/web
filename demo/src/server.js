...
async function init() {
  ...
  server.route({
    ...
    handler: async (request, h) => {
      ...
      try {
        return await platform[claimAction](
          claimMode,
          requestToAPIMapper[claimAction](request)
        );
      } catch (e) {
  ...
  console.log(
    "iProov tokens will be created on %s with API_KEY %s",
    BASE_URL,
    API_KEY
  );
  console.log(
    "Web SDK example available at %s",
    "http://localhost:" + EXAMPLE_SERVER_PORT
  );
}
